...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 23 12:03:11 2018
...
class ImageDataGenerator:
    def __init__(self, class_list, n_class, batch_size = 1, flip = True, \
    ...
    def read_class_list2(self,class_list):
        pass    
    ...
    def getNext_batch(self):
        paths = self.images[self.pointer:self.pointer+self.batch_size] ## 一个batch内的train features
        labels = self.labels[self.pointer:self.pointer+self.batch_size] ## 一个batch内的traun labels
        self.pointer += self.batch_size
        '''
        class_list, n_class, batch_size = 1, flip = True, \
                shuffle = False, mean = np.array([104., 117., 124.]), scale_size = (227,227)
        '''
        ## 生成全是0的 （1，227，227，3）格式数据
        ...
        for i in range(len(paths)):
            ...
            if self.horizontal and np.random.random()<0.5:
                image = cv2.flip(image,1) ## cv2.flip horizontal 水平翻转
